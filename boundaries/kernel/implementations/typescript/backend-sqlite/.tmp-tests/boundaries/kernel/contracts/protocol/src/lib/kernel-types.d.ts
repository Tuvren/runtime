 ...
 */
import type { EpochMs, HashString, KernelObject, KernelRecord } from "@kraken/shared-core-types";
...
export type RunCompletionStatus = Extract<RunStatus, "paused" | "completed" | "failed">;
...
export type Verdict = AbortVerdict | ModifyVerdict | PauseVerdict | ProceedVerdict | RetryVerdict;
...
export type StoredTurnTreePath = StoredChunkedOrderedTurnTreePath | StoredFlatOrderedTurnTreePath | StoredSingleTurnTreePath;
...
export type StoredStagedResult = InterruptedStoredStagedResult | SettledStoredStagedResult;
...
export interface TurnTreePathRepository {
    get(turnTreeHash: HashString, path: string): Promise<StoredTurnTreePath | null>;
...
export interface KrakenKernel {
    branch: {
        create(branchId: string, threadId: string, fromTurnNodeHash: HashString): Promise<BranchRecord>;
    ...
    run: {
        create(runId: string, turnId: string, branchId: string, schemaId: string, startTurnNodeHash: HashString, steps: StepDeclaration[]): Promise<RunRecord>;
        beginStep(runId: string, stepId: string): Promise<StepContext>;
        completeStep(runId: string, stepId: string, eventHash?: HashString, observeResults?: ObserveResult[], treeHash?: HashString): Promise<{
            checkpointed: boolean;
            turnNodeHash?: HashString;
        }>;
        complete(runId: string, status: RunCompletionStatus, eventHash?: HashString): Promise<{
            turnNodeHash?: HashString;
        }>;
    ...
    staging: {
        stage(runId: string, blob: Uint8Array, taskId: string, objectType: string, status: StagedResultStatus, interruptPayload?: KernelRecord): Promise<{
            objectHash: HashString;
    ...
    tree: {
        create(schemaId: string, changes: TurnTreeChangeSet, baseTurnTreeHash?: HashString): Promise<HashString>;
        incorporate(baseTurnTreeHash: HashString, stagedResults: StagedResult[]): Promise<HashString>;
    ...
    turn: {
        create(turnId: string, threadId: string, branchId: string, parentTurnId: string | null | undefined, startTurnNodeHash: HashString): Promise<TurnRecord>;
...
export {};
//# sourceMappingURL=kernel-types.d.ts.map