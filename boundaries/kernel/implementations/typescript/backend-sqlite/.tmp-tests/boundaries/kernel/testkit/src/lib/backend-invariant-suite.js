...
import { KrakenPersistenceError, KrakenValidationError, } from "@kraken/shared-core-types";
import { createCanonicalKernelTestSchema, createCanonicalTurnTreePaths, createHashFromIndex, createStoredObjectRecord, createStoredSchemaRecord, createStoredTurnNodeRecord, createStoredTurnTreeRecord, } from "./kernel-test-fixtures.js";
export function registerBackendInvariantSuite(options) {
    const suiteName = options.suiteName ?? "Backend Invariants";
    options.testApi.describe(suiteName, () => {
        options.testApi.test("preserves deterministic list ordering for branches, runs, and staged results", async () => {
            ...
            const turnTree = await createStoredTurnTreeRecord(schema, {
            ...
            await backend.transact(async (tx) => {
                deepStrictEqual((await tx.branches.listByThread(thread.threadId)).map((branch) => branch.branchId), ["branch_b", "branch_a"]);
                deepStrictEqual((await tx.runs.listByBranch(lateBranch.branchId)).map((run) => run.runId), ["run_a", "run_b"]);
                deepStrictEqual((await tx.stagedResults.listByRun(stagedRun.runId)).map((stagedResult) => stagedResult.taskId), ["task_a", "task_b"]);
            });
        });
        options.testApi.test("rolls back grouped writes when repository validation fails mid-transaction", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            ...
            await rejects(backend.transact(async (tx) => {
                await tx.schemas.put(schemaRecord);
                await tx.turnTrees.put(turnTree);
            ...
            await backend.transact(async (tx) => {
                ...
                deepStrictEqual(await tx.turnTreePaths.get(turnTree.hash, "context.manifest"), null);
            });
        });
        options.testApi.test("rejects thread and turn-node schema mismatches", async () => {
            const backend = options.createBackend();
            ...
            const turnTreeA = await createStoredTurnTreeRecord(schemaA, { "context.manifest": null, messages: [] }, 3);
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(turnTreeA, {
                    "context.manifest": null,
                    messages: [],
                }));
        ...
        options.testApi.test("rejects cross-record reference mismatches for branch, turn, and run state", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            const turnTree = await createStoredTurnTreeRecord(schema, {
            ...
            const siblingTurnTree = await createStoredTurnTreeRecord(schema, {
                "context.manifest": null,
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(siblingTurnTree, {
                    "context.manifest": null,
                    messages: ["9".repeat(64)],
                }));
                await tx.turnNodes.put(turnNode);
        ...
        options.testApi.test("rejects runs with stale branch heads, schema mismatches, duplicate active runs, and invalid initial statuses", async () => {
            const backend = options.createBackend();
            const schemaA = createCanonicalKernelTestSchema();
            const schemaB = {
                ...createCanonicalKernelTestSchema(),
            ...
            const schemaRecordA = createStoredSchemaRecord(schemaA, 1);
            const schemaRecordB = createStoredSchemaRecord(schemaB, 2);
            const turnTree = await createStoredTurnTreeRecord(schemaA, { "context.manifest": null, messages: [] }, 3);
        ...
        options.testApi.test("rejects staged results for non-running runs and keeps staged results immutable per (runId, taskId)", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            const turnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
            const turnNode = await createStoredTurnNodeRecord({
                consumedStagedResults: [],
                createdAtMs: 3,
                eventHash: null,
            ...
            const stagedObject = await createStoredObjectRecord(new Uint8Array([1]), 8);
            const stagedResult = {
                createdAtMs: 9,
            ...
            await rejects(immutableBackend.transact(async (tx) => {
                await tx.stagedResults.set({
                    ...stagedResult,
                    createdAtMs: 10,
                });
        ...
        options.testApi.test("rejects turns whose parent is not the immediate predecessor and rejects ambiguous null parents", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            const turnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
            const siblingTurnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: ["8".repeat(64)] }, 3);
            const rootNode = await createStoredTurnNodeRecord({
                consumedStagedResults: [],
                createdAtMs: 4,
                eventHash: null,
        ...
        options.testApi.test("rejects regressing updatedAtMs on branch, turn, and run updates", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            const turnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
        ...
        options.testApi.test("rejects forged archive branches", async () => {
            const { backend, branch, middleNode, thread } = await createArchiveRollbackScenario(options);
            await rejects(backend.transact(async (tx) => {
                await tx.branches.set({
        ...
        options.testApi.test("rejects stale archival rollback state", async () => {
            const { backend, branch, headNode, middleNode, thread } = await createArchiveRollbackScenario(options);
            await backend.transact(async (tx) => {
                await tx.branches.set({
                    archivedFromBranchId: branch.branchId,
        ...
        options.testApi.test("rejects threads whose root turn node is not genesis", async () => {
            const backend = options.createBackend();
            const { schema, schemaRecord, turnTree, rootNode, middleNode } = await createArchiveRollbackFixtures();
            await rejects(backend.transact(async (tx) => {
                await tx.schemas.put(schemaRecord);
                await tx.turnTrees.put(turnTree);
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(turnTree, {
        ...
        options.testApi.test("rejects turn nodes whose consumed staged results reference missing objects", async () => {
            const backend = options.createBackend();
            const { schema, schemaRecord, turnTree } = await createArchiveRollbackFixtures();
            await rejects(backend.transact(async (tx) => {
                await tx.schemas.put(schemaRecord);
                await tx.turnTrees.put(turnTree);
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(turnTree, {
                ...
                await tx.turnNodes.put(await createStoredTurnNodeRecord({
                    consumedStagedResults: [
                        {
        ...
        options.testApi.test("rejects mismatches between turn tree manifests and indexed path rows", async () => {
            const backend = options.createBackend();
            const { schema, schemaRecord } = await createArchiveRollbackFixtures();
            const turnTreeWithMessage = await createStoredTurnTreeRecord(schema, {
                "context.manifest": null,
        ...
        options.testApi.test("rejects chunked path rows that reference missing chunk records", async () => {
            const backend = options.createBackend();
...
async function createArchiveRollbackFixtures() {
    const schema = createCanonicalKernelTestSchema();
    const schemaRecord = createStoredSchemaRecord(schema, 1);
    const turnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
    const rootNode = await createStoredTurnNodeRecord({
...
async function createArchiveRollbackScenario(options) {
    const backend = options.createBackend();
    ...
    await backend.transact(async (tx) => {
        ...
        await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(fixtures.turnTree, {
            "context.manifest": null,
            messages: [],
        }));
