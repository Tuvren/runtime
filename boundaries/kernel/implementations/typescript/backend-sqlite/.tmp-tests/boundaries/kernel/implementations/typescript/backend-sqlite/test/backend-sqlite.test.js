...
import { createIncrementingClock as createNowClock, delay, registerBackendConformanceSuite, registerBackendInvariantSuite, registerBackendRecoverySuite, } from "@kraken/kernel-testkit";
...
registerBackendConformanceSuite({
    createBackend: () => createSqliteBackend({ databasePath: createTempDatabasePath() }),
...
describe("@kraken/backend-sqlite", () => {
    ...
    test("serializes concurrent transactions and rejects nested transactions", async () => {
        ...
        await rejects(backend.transact(async () => {
    ...
    test("rejects repository handle use after the transaction ends", async () => {
        ...
        await rejects(async () => txHandle.objects.has("0".repeat(64)), KrakenPersistenceError);
    });
});
