...
export function registerBackendRecoverySuite(options) {
    const suiteName = options.suiteName ?? "Backend Recovery";
    options.testApi.describe(suiteName, () => {
        options.testApi.test("supports rollback-safe pause and resume checkpoint flows on the same turn", async () => {
            ...
            const rootTurnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
            ...
            const firstObject = await createStoredObjectRecord(new Uint8Array([1]), 8);
            const firstStagedResult = {
                createdAtMs: 9,
            ...
            const pausedTurnTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [firstObject.hash] }, 10);
            ...
            const secondObject = await createStoredObjectRecord(new Uint8Array([2]), 14);
            const secondStagedResult = {
                createdAtMs: 15,
            ...
            const completedTurnTree = await createStoredTurnTreeRecord(schema, {
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(rootTurnTree, {
                    "context.manifest": null,
                    messages: [],
                }));
            ...
            await rejects(backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(pausedTurnTree, {
                    "context.manifest": null,
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(completedTurnTree, {
                    "context.manifest": null,
                    messages: [firstObject.hash, secondObject.hash],
                }));
            ...
            await backend.transact(async (tx) => {
                deepStrictEqual(await tx.runs.get(resumedRun.runId), completedResumedRun);
                deepStrictEqual(await tx.turnNodes.get(completedTurnNode.hash), completedTurnNode);
                deepStrictEqual(await tx.stagedResults.listByRun(resumedRun.runId), []);
            });
        });
        options.testApi.test("rejects lateral branch moves and backward moves without archival rollback", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            const baseTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
            const siblingTree = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [createHashFromIndex(1)] }, 3);
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(baseTree, {
                    "context.manifest": null,
                    messages: [],
                }));
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(siblingTree, {
                    "context.manifest": null,
        ...
        options.testApi.test("allows backward branch moves when archival rollback semantics are preserved", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            ...
            await backend.transact(async (tx) => {
                ...
                deepStrictEqual(await tx.branches.get(archiveBranch.branchId), archiveBranch);
