...
export function registerBackendConformanceSuite(options) {
    ...
    options.testApi.describe(suiteName, () => {
        ...
        options.testApi.test("rolls back failed transactions without partially visible writes", async () => {
            const backend = options.createBackend();
            const objectRecord = await createStoredObjectRecord(new Uint8Array([1, 2, 3]), 1);
        ...
        options.testApi.test("stores lineage and run-state records with deterministic list and clear helpers", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 100);
            const turnTreeRecord = await createStoredTurnTreeRecord(schema, {
                "context.manifest": null,
                messages: [],
            }, 101);
            const eventObject = await createStoredObjectRecord(new Uint8Array([9, 9, 9]), 102);
            ...
            const stagedObject = await createStoredObjectRecord(new Uint8Array([7, 8, 9]), 109);
            const stagedResult = {
                createdAtMs: 110,
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(turnTreeRecord, {
                    "context.manifest": null,
                    messages: [],
                }));
            ...
            await backend.transact(async (tx) => {
                deepStrictEqual(await tx.turnNodes.get(turnNodeRecord.hash), turnNodeRecord);
                deepStrictEqual(await tx.threads.get(threadRecord.threadId), threadRecord);
                deepStrictEqual(await tx.branches.listByThread(threadRecord.threadId), [branchRecord]);
        ...
        options.testApi.test("accepts idempotent object writes", async () => {
            const backend = options.createBackend();
            const objectRecord = await createStoredObjectRecord(new Uint8Array([4, 5, 6]), 1);
            await backend.transact(async (tx) => {
                await tx.objects.put(objectRecord);
                await tx.objects.put(objectRecord);
            });
        ...
        options.testApi.test("rejects branch heads and archive metadata that cross thread lineage", async () => {
            const backend = options.createBackend();
            const schema = createCanonicalKernelTestSchema();
            const schemaRecord = createStoredSchemaRecord(schema, 1);
            const turnTreeA = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [] }, 2);
            const turnTreeB = await createStoredTurnTreeRecord(schema, { "context.manifest": null, messages: [createHashFromIndex(1)] }, 3);
            ...
            await backend.transact(async (tx) => {
                ...
                await tx.turnTreePaths.putMany(createCanonicalTurnTreePaths(turnTreeB, {
                    "context.manifest": null,
