...
export function isKrakenErrorCode(value) {
    return typeof value === "string" && KRAKEN_ERROR_CODE_PATTERN.test(value);
}
export function assertKrakenErrorCode(value, label = "value") {
    if (!isKrakenErrorCode(value)) {
        throw new TypeError(`${label} must be a lowercase snake_case Kraken error code`);
    }
}
