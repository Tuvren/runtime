...
- **Planning Note — this is a planning branch:** The SaaS-readiness specs (`docs/KrakenKernelSpecification.md` v0.12, `docs/KrakenFrameworkSpecification.md` v0.21) carry the target semantics, but no implementation source, schema, generated artifact, conformance plan, or authority packet has been changed yet. The docs-to-authority freeze gate classifies the new sections honestly — version-status headers as `explicitly-deferred` and each target claim as `missing-conformance-follow-up` tracked to its owning epic's first ticket — so the gate and `bun run verify` pass **green** on this branch with no false conformance coverage asserted for unimplemented semantics. Each epic's first ticket (`KRT-BE001`, `KRT-BF001`, `KRT-BG001`, `KRT-BH001`) is the authority-alignment ticket: it declares the new surface and its conformance-plan reference in the owning boundary authority packet (e.g. `KRT-BE001` declares the kernel-protocol `bindingSections.scope-isolation` surface and the `kernel-scope-isolation.json` contract). Promotion of those spec sections from `missing-conformance-follow-up` to `authority-backed-conformance-covered` happens only when the epic's conformance ticket makes the plan runnable, registered, and evidence-backed (for Epic BE that is `KRT-BE007`) — the portability gate (`plan-applicability-has-adapter`) and the compatibility evidence gate forbid registering a runnable plan before an adapter can execute it, so promoting in the first ticket would assert coverage that does not yet exist.
...
- Each epic's first ticket (`KRT-BE001`, `KRT-BF001`, `KRT-BG001`, `KRT-BH001`) has promoted the docs-to-authority coverage matrix, authority packets, and conformance plans for the kernel v0.12 / framework v0.21 spec sections it implements from `missing-conformance-follow-up` to `authority-backed-conformance-covered`, so `bun run verify` exits zero from a clean checkout once the block is complete (the gate is already green on the planning branch via honest deferred / missing-conformance classification before the block executes).
