...
- **Total Active Story Points:** 60 (**60 remaining**) — with the data-lifecycle **Epic BF (Reclamation + Crypto-Shredding Erasure, 39 pts) now closed** (following the tenancy keystone Epic BE), the remaining **SaaS-Readiness block (Epics BG–BI)** is the active execution plan that lets any host embed Tuvren as a multi-tenant SaaS substrate without forking the runtime. Per-epic: BG 23, BH 16, BI 21. Everything through Epic BF (the Tooling block AW–BC, the trust block BD, the tenancy keystone BE, and the data-lifecycle epic BF) remains closed and is retained as a compact audit ledger below.
- **Critical Path:** `KRT-BG001 → KRT-BG002 → KRT-BG003 → KRT-BG004 → KRT-BG005 → KRT-BH001 → KRT-BH002 → KRT-BH003 → KRT-BH004 → KRT-BH005 → KRT-BI001 → KRT-BI002 → KRT-BI003 → KRT-BI004 → KRT-BI005`. With the keystone Epic BE (tenancy scope + isolation) and the data-lifecycle Epic BF (reclamation + crypto-shredding erasure) closed, the active critical path begins at the lease-clock epic (BG); the SDK freeze epic (BI) per ADR-054 (freeze-after-tenancy+GC) now has both of its gating epics (BE + BF) landed. BG (lease clock + side-effect-once) and BH (conversation-state ownership) branch off the closed BE and BF and rejoin before BI. Within BG, `KRT-BG002` and `KRT-BG003` may run in parallel.
- **Planning Note — partially implemented branch:** Epics BE and BF have **landed**: BE's kernel `docs/KrakenKernelSpecification.md` §2.3 scope-resolved-identity claims and the framework scope notes, and BF's §9.4 `maintenance.reclamation` reachability-primitive claims, are promoted from `missing-conformance-follow-up` to `authority-backed-conformance-covered` (the kernel-protocol authority packet declares `bindingSections.scope-isolation` and `bindingSections.reclamation` and registers the runnable, evidence-backed `kernel-scope-isolation.json` and `kernel-reclamation.json` plans; the shared-core operational-telemetry plan proves scope-correlated telemetry/transcripts, and BF's reclaim-probe + erasure-probe carry per-backend evidence across memory/SQLite/PostgreSQL), with implementation source, schema, generated artifacts, conformance plans, and refreshed compatibility evidence all changed accordingly. The §9.4 non-support row (a backend advertising `maintenance.reclamation=false` rejecting with `kernel_capability_unsupported`) stays classified with the rest of the capability-gating contract (`missing-conformance-follow-up`, tracked to `KRT-AM010`), since the promoted plan only exercises capability-bearing backends. The remaining SaaS-readiness specs for BG–BH (`docs/KrakenFrameworkSpecification.md` v0.21 lease-clock/side-effect-once/conversation-state notes) still carry target semantics with no implementation yet; the docs-to-authority freeze gate classifies those sections honestly — version-status headers as `explicitly-deferred` and each remaining target claim as `missing-conformance-follow-up` tracked to its owning epic's first ticket (`KRT-BG001`, `KRT-BH001`) — so the gate and `bun run verify` pass **green** with no false conformance coverage for unimplemented semantics. Each remaining epic's first ticket is the authority-alignment ticket that declares the new surface and its conformance-plan reference in the owning boundary authority packet; promotion to `authority-backed-conformance-covered` happens only when the epic's conformance ticket makes the plan runnable, registered, and evidence-backed (as BE and BF demonstrated — `KRT-BE001`/`KRT-BF001` declared the surfaces and `KRT-BE007`/`KRT-BF007` promoted them), because the portability gate (`plan-applicability-has-adapter`) and the compatibility evidence gate forbid registering a runnable plan before an adapter can execute it.
...
- **Block 6 — SaaS-Readiness (Epics BG–BI active; BE + BF closed): ACTIVE.** The runtime is functionally complete and production-trust-hardened; this block makes it safe and stable to embed as a multi-tenant SaaS substrate without forking, governed by PRD v0.10.0 (CAP-P0-064 through CAP-P0-070), Architecture v0.10.0, and TechSpec v0.30.x (ADR-048 through ADR-055). Tuvren stays tenancy-agnostic: it provides the mechanism (scope seam, isolation, reclamation, erasure, side-effect-once, conversation-state ownership, a published stable SDK), and the host owns tenancy/retention policy and keys. The keystone Epic BE and the data-lifecycle Epic BF have landed; Epics BG–BI remain active.
...
- The Tooling block (Epics AW–BC), the trust block (Epic BD, Trust-Boundary Security Hardening), the tenancy keystone (Epic BE, Tenancy Scope Seam + Isolation-by-Construction), and the data-lifecycle epic (Epic BF, Reclamation + Crypto-Shredding Erasure) are fully closed. The active execution plan is now the remainder of the SaaS-Readiness block (Epics BG–BI), which recasts the previously named-but-unticketed productionization roadmap into ticketed work; see Current Active Scope and the Post-SaaS-Readiness Roadmap (Epics BJ–BM).

## 3. Build Order (Mermaid)

Active SaaS-Readiness block only (Epics AM–BF are closed; Epics BJ–BM are deferred and excluded).

```mermaid
flowchart LR
  priorClosed["Epics AM–BF — CLOSED (incl. data-lifecycle BF)"]
...
- Each epic's first ticket (`KRT-BE001`, `KRT-BF001` done; `KRT-BG001`, `KRT-BH001` pending) promotes the docs-to-authority coverage matrix, authority packets, and conformance plans for the kernel v0.12 / framework v0.21 spec sections it implements from `missing-conformance-follow-up` to `authority-backed-conformance-covered`, so `bun run verify` exits zero from a clean checkout once the block is complete. Epics BE and BF have completed this cascade (BE's §2.3 scope-isolation and BF's §9.4 reclamation claims are authority-backed and evidence-covered; the §9.4 non-support rejection row stays with the capability-gating contract tracked to `KRT-AM010`); the remaining BG–BH sections stay honestly classified as deferred / missing-conformance until their epics execute.
