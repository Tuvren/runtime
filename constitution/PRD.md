...
| Operational Telemetry | The first-class, durable, correlated record of what a turn did (model interactions, tool calls, checkpoints, approvals, recovery events, errors), keyed to runtime lineage concepts, used for operating and debugging the runtime in production and distinct from the real-time host event stream. | logs, metrics blob, the event stream |
...
- **Capability:** The product must enforce bounded execution so that a single turn cannot run unbounded iterations, tool calls, or resource consumption; when a configured bound is reached, the runtime must stop safely and surface the outcome as a host-visible and agent-visible result rather than looping or exhausting resources silently.
