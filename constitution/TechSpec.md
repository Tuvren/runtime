...
- **Current host-proof posture:** The repository still proves the host story mainly through the private playground harness and its scenario lanes, but that harness now consumes the curated `@tuvren/runtime` facade instead of the prior lower-level runtime package set. That is sufficient as current-state evidence that the runtime is real, but it is not yet the serious REPL proving host required by the active TypeScript productization posture.
- **Current package/publication posture:** A first curated host-facing SDK surface now exists as `@tuvren/runtime`, and current host-building paths consume it rather than reaching into `runtime-core`, `runtime-api`, `driver-react`, `kernel-runtime`, `core-types`, and `kernel-protocol` directly. Final public package publication, naming, and long-lived curation may still evolve after the serious REPL host lands.
