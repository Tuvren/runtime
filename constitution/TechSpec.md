...
- **Epic AB — Run Liveness and Stale-Running Recovery:** Landed in current repo reality as a contract-first extension. The TypeScript kernel, memory/SQLite backends, shared runtime-core, promoted shared framework stale-recovery checks, and refreshed compatibility artifacts now align on leased running ownership and stale-run preemption as optional advertised capabilities rather than a hidden change to the frozen 28-operation surface. Rust remains non-applicable unless it advertises the relevant liveness capability.
