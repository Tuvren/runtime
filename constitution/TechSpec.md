...
- v0.9.1 - Hardened the TypeScript provider and framework compatibility boundary by replacing overbroad provider capability labels with framework-mediated contract labels, promoting fail-closed strict-structured-output and provider-owned tool or approval rejection checks into shared conformance, and refreshing compatibility evidence to report the narrowed TypeScript guarantees explicitly.
...
- **Current framework orchestration authority gap:** The framework spec, the TypeScript binding appendix, and `runtime-core` tests already define orchestration semantics such as `spawn()`, `allEvents()`, `awaitResult()`, descendant source attribution, and run-local worker lifecycle behavior. That surface is not yet promoted into machine-owned authority: the runtime-api Authority Packet does not reference orchestration plans, the runtime-api TypeSpec source does not model orchestration runtime or handle shapes, the shared framework conformance plans do not yet assert orchestration behavior as first-class checks, the TypeScript framework adapter does not advertise `framework.orchestration`, and the compatibility matrix does not report the capability.
...
16. Epic AC promotes framework orchestration from TypeScript-local and prose-described behavior into boundary-owned authority and shared-runner evidence. The active work is authority inventory, runtime-api packet or packet-family extension, shared orchestration plans, TypeScript adapter operations, and compatibility evidence refresh for `framework.orchestration`.
17. Add a Rust framework implementation, future concrete drivers, peer backends such as PostgreSQL and MySQL/MariaDB, provider expansions, or additional host protocols only after Epic AC lands or is explicitly superseded by a later TechSpec revision.
...
- **Epic AC — Framework Orchestration Authority Promotion:** Active. This epic promotes orchestration semantics already specified in `docs/KrakenFrameworkSpecification.md` §10 and exercised by TypeScript `runtime-core` into runtime-api authority, shared conformance plans, TypeScript adapter capability evidence, and compatibility reporting, without introducing new kernel concepts, a global worker scheduler, A2A/ACP, provider-native handoff/tool routing, or Rust framework implementation work.
- **Later driver, backend, provider, language, and host protocol expansion:** Deferred beyond Epic AC unless the TechSpec is revised. This includes Rust framework work, `LanguageModelV2` compatibility, provider-native tools, AI SDK agent loops/UI transports, LangChain bridge work, first-class Tuvren provider packages, ACP, future non-ReAct drivers, future official backends, and future language lines beyond Rust.
