...
- **Freeze-readiness posture:** TypeScript freeze-readiness is reaffirmed for the currently promoted supported applicable surfaces after Epic AG. The reaffirmation is backed by fresh AG-gated `bun run verify`, `bun run release-check`, `bun run conformance`, `bun run codegen`, `bun run compatibility:evidence`, and `bun run interop-smoke` evidence plus regenerated compatibility evidence. Rust framework remains unsupported, and this reaffirmation does not activate any new implementation line by itself. Future readiness claims must be regenerated through the same AG-gated evidence stack whenever promoted surfaces change materially.
...
- **Current freeze-readiness posture:** `reports/compatibility/compatibility-matrix.json` now records fresh AG-gated evidence for the promoted surfaces, and that evidence reaffirms TypeScript freeze-readiness for the currently promoted supported applicable surfaces. Epic AD classifies framework and kernel docs claims in `constitution/spikes/epic-ad-docs-to-authority-coverage-matrix.json`, Epic AE records modular hardening in `constitution/spikes/epic-ae-modular-boundary-hardening-inventory.md`, and Epic AF remains the archived historical closure inventory rather than the active readiness claim.
...
- **Current implementation-line gate posture:** Rust framework product work remains blocked until a later TechSpec revision explicitly activates it. TypeScript freeze-readiness is reaffirmed only for the currently promoted supported applicable surfaces, and that reaffirmation does not activate any later framework, driver, backend, provider, language, or host protocol line by itself.
...
- **Migration Notes:** Existing TypeScript testkit packages remain implementation-local helper/facade packages under `boundaries/<area>/implementations/typescript/testkit/`. Promoted compatibility evidence now flows through the shared semantic runner and implementation adapter hosts, not implementation-specific semantic runners. Epics AD through AG are archived historical context, and the AG closure now defines the live readiness baseline. Historical closure inventories may inform future maintenance, but current readiness claims must be generated from live checks or removed.
...
19. Epic AG is closed in current repo reality. It hardened non-self-attesting conformance across authority packets, conformance plans, implementation adapters, the generic runner, compatibility reporting, and live closure claims, and it reaffirmed TypeScript freeze-readiness for the currently promoted supported applicable surfaces under fresh AG-gated evidence. Rust framework remains unsupported unless a later TechSpec revision activates real product behavior.
