...
17. Add a Rust framework implementation, future concrete drivers, peer backends such as PostgreSQL and MySQL/MariaDB, provider expansions, or additional host protocols only after closed Epic AC is intentionally extended or superseded by a later TechSpec revision.
...
- **Later driver, backend, provider, language, and host protocol expansion:** Deferred beyond closed Epic AC unless the TechSpec is revised. This includes Rust framework work, `LanguageModelV2` compatibility, provider-native tools, AI SDK agent loops/UI transports, LangChain bridge work, first-class Tuvren provider packages, ACP, future non-ReAct drivers, future official backends, and future language lines beyond Rust.
