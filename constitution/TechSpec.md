...
- **Current capability-orchestration posture:** Epics AW, AX, and AY are closed. Repository reality now includes: `@tuvren/core/capabilities` subpath with all §3.13 types plus `CapabilityPolicyEngine` and `TuvrenSandboxExecutor` interfaces; Capability Registry, Binding & Endpoint Resolver (back-compat `defineTool` → `tuvren-server / tuvren-in-process`; MCP → `tuvren-server / mcp-server`; `metadata.sandbox.endpointId` → `tuvren-server / tuvren-sandbox`), and Capability Policy Engine wired into tool dispatch; `TuvrenToolDefinition` extended with `outputSchema?`, `idempotent?`, `maxRetries?`; `AgentConfig` extended with `capabilityPolicyEngine?`, `serverExecution?` (rate-limit config), `sandboxExecutors?`, `providerNativeTools?`, and `providerMediatedTools?`; `TOOL_INPUT_VALIDATION_FAILED`, `TOOL_RESULT_VALIDATION_FAILED`, `TOOL_INVOCATION_RATE_LIMITED` error codes in `@tuvren/core/errors`; `ServerRateLimiter` (fixed-window, per-turn, per-runtime-instance) for tenant-scoped call-budget enforcement; `ToolAuditEvent` (`tool.audit`) in `TuvrenStreamEvent` emitting at five lifecycle points with no secret material; execution-class + owner attribution on events and telemetry; `TuvrenPrompt` extended with `providerNativeTools?`, `providerMediatedTools?`, `providerContinuity?`; `TuvrenModelResponse` extended with `providerToolResults?`; pre-staged provider tool messages bypass the Tool Execution Gateway; `emitProviderToolAttributionEvents` emits `tool.start`/`tool.result` with `owner:"provider"` and per-class observation limits (canAudit/canCancel/canRetry/canResume: false, canPersistResult: true); `tuvren-server-execution-class` conformance check set (19 checks, 19/19 pass); `provider-native-execution-class` (10 checks, 10/10 pass) and `provider-mediated-execution-class` (10 checks, 10/10 pass) in the `tuvren.providers.provider-api` authority packet. 358 runtime tests + 54 bridge tests + 78 react-driver tests pass; 52/52 provider conformance checks pass. The remaining Tooling block (Epics AZ–BC) lands the Tuvren-client execution class and closes out the cross-class model.
