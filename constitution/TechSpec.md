...
- **Epic AB — Run Liveness and Stale-Running Recovery:** Landed in current repo reality as a contract-first extension. The TypeScript kernel, memory/SQLite backends, shared runtime-core, shared-runner conformance plan, and refreshed compatibility artifacts now align on leased running ownership and stale-run preemption as an optional advertised capability rather than a hidden change to the frozen 28-operation surface. Rust remains non-applicable unless it advertises `kernel.run-liveness`.
