...
- **Freeze-readiness posture:** Passing the currently promoted conformance subsets is necessary but not sufficient for TypeScript freeze-readiness. Epic AD now supplies the checked-in docs-to-authority coverage matrix and explicit classification of remaining implementation-local, implementation-defined, deferred, stale-corrected, or missing-conformance surfaces. Epic AE has completed the large TypeScript semantic-gravity-well decomposition behind stable seams. Freeze status still requires fresh `bun run verify`, `bun run release-check`, `bun run conformance`, `bun run codegen`, and `bun run interop-smoke` evidence from a clean checkout after Epic AF closes.
...
- **Current freeze-readiness posture:** `reports/compatibility/compatibility-matrix.json` currently records `typescript-framework` as `full_pass` for `233/233` shared framework checks, `typescript-kernel-sqlite` as `full_pass` for `45/45` kernel checks, `typescript-providers` as `full_pass` for `28/28` provider checks, and `rust-framework` as `unsupported` with `233` non-applicable checks. Epic AD now classifies the framework and kernel docs claims in `constitution/spikes/epic-ad-docs-to-authority-coverage-matrix.json`, and Epic AE records the completed modular-hardening closure in `constitution/spikes/epic-ae-modular-boundary-hardening-inventory.md`. That evidence does not yet prove that TypeScript is freeze-ready because Epic AF remains open.
...
- **Current implementation-line gate posture:** Rust framework product work remains blocked. Epics AD and AE are closed as docs-to-authority and modular-hardening gates, but the current authorized implementation scope still requires Epic AF before TypeScript can be recorded as freeze-ready. Only a later TechSpec revision may activate a new framework implementation line after that closure.
...
- **Decision:** The next implementation scope after closed Epic AC is the TypeScript freeze-readiness closure. Epic AD has classified every normative claim in `docs/KrakenFrameworkSpecification.md` and `docs/KrakenKernelSpecification.md` and labeled remaining implementation-local, implementation-defined, explicitly deferred, stale-corrected, or missing-conformance claims. Before any new framework implementation line, future driver line, future official backend line, or future host/protocol expansion is activated, the repository must still split the identified large TypeScript semantic gravity wells behind stable seams without public API churn, promote only the intended portable negative/interleaving behavior into shared conformance, and regenerate clean `verify`, `release-check`, `conformance`, `codegen`, and `interop-smoke` evidence.
- **Consequences:** Epic AD is closed and Epics AE and AF remain the active implementation scope in `Tasks.md`. Rust framework remains evidence-only and unsupported until that closure is complete and a later TechSpec revision intentionally activates product behavior. A `full_pass` compatibility entry is necessary evidence for freeze-readiness, but it is not sufficient by itself without the docs-to-authority matrix, modular-hardening results, and clean-checkout evidence.
...
18. The remaining authorized implementation scope after closed Epic AD is the TypeScript freeze-readiness closure through Epics AE and AF: harden the large TypeScript structural seams without public API churn, deepen shared conformance only for intended portable behavior, add drift guardrails, and regenerate clean evidence.
19. Rust framework implementation, future framework language lines, future concrete drivers, peer backends such as PostgreSQL and MySQL/MariaDB, provider expansions, or additional host protocols remain blocked until the freeze-readiness closure records TypeScript as freeze-ready through fresh `bun run verify`, `bun run release-check`, `bun run conformance`, `bun run codegen`, and `bun run interop-smoke` evidence, and a later TechSpec revision intentionally activates the next line.
...
- **Later driver, backend, provider, language, and host protocol expansion:** Deferred beyond active Epic AF unless the TechSpec is revised again. This includes Rust framework work, `LanguageModelV2` compatibility, provider-native tools, AI SDK agent loops/UI transports, LangChain bridge work, first-class Tuvren provider packages, ACP, future non-ReAct drivers, future official backends, and future language lines beyond Rust.
