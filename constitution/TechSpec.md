...
- **Current capability-orchestration posture:** Epics AW, AX, AY, and AZ are closed. Repository reality now includes: `@tuvren/core/capabilities` subpath with all §3.13 types plus `CapabilityPolicyEngine`, `TuvrenSandboxExecutor`, `ClientEndpointBoundary`, and `ClientDispatchResult` interfaces; `AttachedClientEndpoint`, `ClientEndpointCapabilityAdvertisement`, `ClientInvocationEnvelope`, `ClientReportedResult` shapes; Capability Registry, Binding & Endpoint Resolver (back-compat `defineTool` → `tuvren-server / tuvren-in-process`; MCP → `tuvren-server / mcp-server`; `metadata.sandbox.endpointId` → `tuvren-server / tuvren-sandbox`; `metadata.clientEndpointId` → `tuvren-client / client-endpoint` or `tuvren-client / mcp-server` for client-side MCP), and Capability Policy Engine wired into tool dispatch; `TuvrenToolDefinition` extended with `outputSchema?`, `idempotent?`, `maxRetries?`; `AgentConfig` extended with `capabilityPolicyEngine?`, `serverExecution?`, `sandboxExecutors?`, `providerNativeTools?`, `providerMediatedTools?`, `clientEndpoints?`, and `clientEndpointBoundary?`; `isClientEndpointTool` guard suppresses `tool.audit` events and server-side rate-limiting for the tuvren-client class; `observationForClass("tuvren-client")` explicit; `ClientEndpointBoundary.detach()` supports dynamic endpoint lifecycle; `PauseContext` and `LoopState` carry the boundary through pause/resume and handoff cycles; `createClientEndpointBoundary` exported from `@tuvren/runtime`; `tuvren-server-execution-class` conformance check set (19 checks, 19/19 pass); `provider-native-execution-class` (10 checks, 10/10 pass) and `provider-mediated-execution-class` (10 checks, 10/10 pass) in the `tuvren.providers.provider-api` authority packet; `tuvren-client-execution-class` conformance check set (13 checks, 13/13 pass) in the `tuvren.shared.core` authority packet. 394 runtime tests + 54 bridge tests + 78 react-driver tests pass; 52/52 provider + 13/13 client conformance checks = 379/379 framework checks pass; `verify:kernel:fresh` passes. The remaining Tooling block (Epics BA–BC) closes out the cross-class model.
...
- **Error model:** Exposure denials remove a surface from the model-visible set (no error to the model) — the enforcement filtering the tool-set before the model sees it is active as of Epic BB (via `applyExposureFilter` in the driver tool registry path). Invocation denials and unavailable bindings surface as `tool.result` with `isError: true` carrying the appropriate existing error family — `TuvrenValidationError` for local input-contract violations (`tool_input_validation_failed`), `TuvrenProviderError` for provider/mediated/MCP-advertised failures (e.g. `mcp_tool_input_invalid`), and a new `capability_binding_unavailable` `TuvrenRuntimeError` when no admissible binding exists (for example a Tuvren-client endpoint is not attached). Provider-native invocation failures are recorded from provider-exposed results.

- The runtime resolves each model-visible tool call through: Capability Registry (which surfaces are eligible) → Capability Policy Engine (exposure-time; active as of Epic BB) → the model sees only exposed surfaces → on a tool call, Binding & Endpoint Resolver (resolve capability → execution class + endpoint) → Capability Policy Engine (invocation-time; active as of Epic AW, full policy depth as of Epic BB) → dispatch to the execution-class endpoint. This preserves the conceptual invariant.
- Exposure-time policy inputs: provider, model, user/org permissions, data-residency, endpoint availability. Invocation-time policy inputs: approval requirements, credential boundary, user-presence, idempotency/retry, risk classification. Both are framework-owned decision points above driver discretion (consistent with the §4.19 bounds guard and the §5.6.3 approval/secret rules). **Wiring scope note (as of Epic BB):** engine-creation-time options (`allowedRegions`, `maxAllowedRiskClass`, `enforceCredentialBoundary`, `deniedCapabilityIds`) are fully active at both decision points. Turn-level context signals (`userPresent`, `endpointAttached`, `entitledCredentialScopes`) are _not_ auto-populated by the runtime; presence and credential-boundary dimensions that depend on them require a custom `CapabilityPolicyEngine` or a future `AgentConfig` extension that threads per-turn signals. Policy metadata (`riskClass`, `idempotencyPolicy`, `credentialScope`, `endpointRegion`) is threaded for in-process (`tuvren-server`) bindings; MCP-server, sandbox, and client-endpoint binding kinds do not yet carry policy metadata. `requiresApproval` decisions from `RiskDimension` are surfaced as hard denials at the runtime level; routing them to the tool-approval queue is deferred.
...
- **Epic BB — Exposure & Invocation Policy Model: CLOSED.** Landed: five policy dimensions (`ResidencyDimension`, `RiskDimension`, `PresenceDimension`, `CredentialBoundaryDimension`, `DenyListDimension`) in a `PolicyDimension` pipeline with deterministic first-deny-wins composition; `PolicyDimension` extension interface exported from `@tuvren/runtime`; exposure-time filtering active (`applyExposureFilter` in the driver tool registry snapshot path); `policyCanRetry` annotation wired into `executeSingleTool` retry path; policy-relevant metadata (`endpointRegion`, `riskClass`, `requiresUserPresence`, `idempotencyPolicy`, `credentialScope`) threaded through `resolveFromToolDefinition` and the exposure-filter surface map for in-process tools; all five dimension-specific fields added to `CapabilityPolicyContext`, `ToolSurface`, `Endpoint`, `Binding`, and `InvocationDecision` (all optional/additive); 26-check `capability-policy` conformance plan (BB001–BB005 coverage, both decision points, non-secret reasons, determinism) registered as an executable verification path; 478 runtime tests pass; 425/425 framework conformance checks pass; `bun run verify` exits 0. **Known scope boundaries:** (1) turn-level context signals (`userPresent`, `endpointAttached`, `entitledCredentialScopes`) are not auto-populated by the runtime — presence and credential-boundary dimensions are active at the engine API but require a custom engine or future `AgentConfig` extension to thread per-turn state; (2) policy metadata is threaded for in-process tuvren-server bindings only (MCP-server, sandbox, and client-endpoint binding kinds are out of scope for this epic); (3) `requiresApproval` decisions surface as hard denials at the runtime level — routing to the tool-approval queue is deferred.
