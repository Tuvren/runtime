...
Order:
1. Add `ExecutionBounds` and `ExecutionBoundExceededDetails` (§3.11) to `@tuvren/core/execution`; document the `execution_bound_exceeded` code in `@tuvren/core/errors`; and update the shared core machine-readable sources, generated artifacts, and merged core authority packet for the new execution contract, including a packet-version bump.
