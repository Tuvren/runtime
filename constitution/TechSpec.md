...
- **Epic Y — Machine-Enforced Neutral Authority Closure:** Closed in current repo reality through `constitution/spikes/epic-y-authority-leak-inventory.md` and `constitution/spikes/epic-y-machine-enforced-authority-closure-inventory.md`. The named surfaces now own Authority Packet manifests, conformance plans, generated JSON Schema artifacts, binding appendices, validator wiring, and machine guardrails. Existing TypeScript public package APIs remain binding projections of those packets.
