...
- **Decision:** Make secret isolation an explicit, enforced contract:
  ...
  4. **Verification.** The absence of secret material on durable, telemetry, and transcript surfaces is asserted by conformance and review, not by redaction alone. A `secret-isolation` check set added to the relevant plans drives a fixture that configures a provider key and an MCP bearer token, runs a turn, and asserts none of the configured secret values appear in persisted kernel records, captured telemetry attributes or error summaries, or a recorded transcript.
- **Consequences:** `@tuvren/mcp-client` and `@tuvren/provider-bridge-ai-sdk` document the edge-confinement rule and add it to their package READMEs and conformance fixtures. `@tuvren/repl-host`'s `repl-transcript.ts` gains the backend-options redactor and a non-secret backend identity descriptor in the transcript header path (§3.9 constraint). The telemetry sink path in `@tuvren/runtime` applies the attribute allowlist and telemetry-error sanitization. The `secret-isolation` check set is added to `providers-mcp-client.json` (MCP auth path), `framework-operational-telemetry.json` (telemetry attributes and error summaries), and `runtime-api-callables-extended.json` (transcript redaction). No new runtime error code is introduced; a detected leak in verification fails the relevant conformance check rather than throwing at runtime.
...
- **Observability Hooks:**
  ...
  - per ADR-044, no secret material may reach the telemetry sink, durable kernel records, or transcripts; host-supplied telemetry attributes pass through a semconv allowlist, telemetry error summaries are sanitized before emission, and transcript headers redact backend credential fields
...
4. Add the `secret-isolation` check set to `providers-mcp-client.json`, `framework-operational-telemetry.json`, and `runtime-api-callables-extended.json`: configure a provider key and MCP bearer token, run a turn, and assert none of the configured secret values appear in persisted records, captured telemetry attributes or error summaries, or a recorded transcript.
