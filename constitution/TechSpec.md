...
- **Freeze-readiness posture:** Passing the currently promoted conformance subsets is necessary but not sufficient for TypeScript freeze-readiness. Freeze status additionally requires a checked-in docs-to-authority coverage matrix, explicit classification of remaining implementation-local or deferred surfaces, decomposition of the large TypeScript semantic gravity wells behind stable seams, and fresh `bun run verify`, `bun run release-check`, `bun run conformance`, `bun run codegen`, and `bun run interop-smoke` evidence from a clean checkout.
...
- **Current freeze-readiness posture:** `reports/compatibility/compatibility-matrix.json` currently records `typescript-framework` as `full_pass` for `233/233` shared framework checks, `typescript-kernel-sqlite` as `full_pass` for `45/45` kernel checks, `typescript-providers` as `full_pass` for `28/28` provider checks, and `rust-framework` as `unsupported` with `233` non-applicable checks. That proves the promoted authority subset is green; it does not yet prove that a future implementation maintainer can ignore TypeScript source and TypeScript tests safely.
...
- **Current framework orchestration authority posture:** The framework spec, the runtime-api authority packet, the shared orchestration conformance plan, the TypeScript binding appendix, and refreshed compatibility evidence now align on orchestration semantics such as `spawn()`, `allEvents()`, `awaitResult()`, descendant source attribution, run-local worker lifecycle behavior, explicit execution-surface inheritance, and nested descendant attribution. Static config snapshotting and extension scoping remain local implementation evidence until the freeze-readiness scope classifies them explicitly, and TypeScript `runtime-core` tests remain implementation evidence only.
- **Current implementation-line gate posture:** Rust framework product work remains blocked. The next authorized implementation scope is TypeScript freeze-readiness closure through Epics AD, AE, and AF; only a later TechSpec revision may activate a new framework implementation line after that closure records TypeScript as freeze-ready.
...
### ADR-029 TypeScript Freeze-Readiness Closure Gates New Framework Implementation Lines

- **Status:** accepted
- **Context:** The current compatibility matrix proves that the promoted framework, kernel, and provider subsets are green for TypeScript, and that Rust framework remains unsupported. That is strong evidence, but it is still possible for a future implementation maintainer to discover required behavior faster by reading large TypeScript modules or TypeScript tests than by reading authority packets, plans, fixtures, and measured evidence alone. Activating Rust framework work from that posture would reintroduce `runtime-core`, `sqlite-backend`, the framework adapter, provider bridge, or their oversized tests as hidden semantic or structural oracles.
- **Decision:** The next implementation scope after closed Epic AC is the TypeScript freeze-readiness closure. Before any new framework implementation line, future driver line, future official backend line, or future host/protocol expansion is activated, the repository must: classify every normative claim in `docs/KrakenFrameworkSpecification.md` and `docs/KrakenKernelSpecification.md`; label every remaining implementation-defined, explicitly deferred, or stale claim; split the identified large TypeScript semantic gravity wells behind stable seams without public API churn; promote only the intended portable negative/interleaving behavior into shared conformance; and regenerate clean `verify`, `release-check`, `conformance`, `codegen`, and `interop-smoke` evidence.
- **Consequences:** Epics AD, AE, and AF become the active implementation scope in `Tasks.md`. Rust framework remains evidence-only and unsupported until that closure is complete and a later TechSpec revision intentionally activates product behavior. A `full_pass` compatibility entry is necessary evidence for freeze-readiness, but it is not sufficient by itself without the docs-to-authority matrix, modular-hardening results, and clean-checkout evidence.
...
4. Epic S is closed: kernel protocol CDDL grammar exists, and TypeScript conformance evidence now flows through the then-current implementation-specific runners over boundary-owned conformance assets; final Epic Y closure migrates that evidence path toward the shared semantic runner.
...
13. Epic Z is closed: `@tuvren/kernel-runtime` now exists as the boundary-owned TypeScript `RuntimeKernel` implementation over the existing `RuntimeBackend` contract, and private playground-local syscall semantics have been removed or reduced to host-owned inspectors.
14. Epic AA is closed: the kernel protocol authority packet now references the promoted plan family, the TypeScript memory and SQLite kernel adapters execute native `@tuvren/kernel-runtime` behavior under the shared runner, and logical checks, lineage checks, verdict composition, run-liveness checks, and restart-recovery checks are shared-runner evidence rather than TypeScript-local assertions.
...
17. The next authorized implementation scope after closed Epic AC is the TypeScript freeze-readiness closure: Epics AD, AE, and AF classify normative docs claims, label local/deferred semantics, harden the large TypeScript structural seams without public API churn, deepen shared conformance only for intended portable behavior, and regenerate clean evidence.
18. Rust framework implementation, future framework language lines, future concrete drivers, peer backends such as PostgreSQL and MySQL/MariaDB, provider expansions, or additional host protocols remain blocked until the freeze-readiness closure records TypeScript as freeze-ready through fresh `bun run verify`, `bun run release-check`, `bun run conformance`, `bun run codegen`, and `bun run interop-smoke` evidence, and a later TechSpec revision intentionally activates the next line.
...
- **Epic Z — TypeScript Kernel Syscall Closure:** Closed in current repo reality through `constitution/spikes/epic-z-typescript-kernel-syscall-closure-inventory.md`. The repo now has `@tuvren/kernel-runtime` under `boundaries/kernel/implementations/typescript/runtime-kernel`, `createRuntimeKernel()` as the boundary-owned `RuntimeBackend` to `RuntimeKernel` adapter, and playground local memory/SQLite modes wired through that package rather than private playground syscall logic.
- **Epic AA — Kernel Conformance Promotion:** Closed in current repo reality through `constitution/spikes/epic-aa-kernel-conformance-promotion-closure-inventory.md`. The kernel protocol packet now names promoted kernel protocol, run-liveness, and restart-recovery plans; TypeScript kernel adapters execute native `@tuvren/kernel-runtime`; logical, lineage, verdict, run-liveness, and restart-recovery coverage now comes from shared-runner evidence; and Rust remains capability-scoped where extensions are not advertised.
...
- **Epic AD — Docs-to-Authority Freeze Gate:** Active scope. The next planning layer must classify every normative framework and kernel docs claim against authority packets, conformance plans, fixtures, compatibility evidence, implementation-local evidence, deferral, or staleness before any new framework implementation line is activated.
- **Epic AE — TypeScript Modular Boundary Hardening:** Active scope. The next implementation layer must split the large TypeScript semantic gravity wells behind stable seams so maintainability improves without turning refactors into hidden public API or semantic changes.
- **Epic AF — Conformance Depth and Freeze Evidence:** Active scope. The next evidence layer must promote the intended portable negative/interleaving behavior, add guardrails that prevent authority drift from returning, and certify whether TypeScript is freeze-ready from a clean checkout.
- **Later driver, backend, provider, language, and host protocol expansion:** Deferred beyond active Epics AD, AE, and AF unless the TechSpec is revised again. This includes Rust framework work, `LanguageModelV2` compatibility, provider-native tools, AI SDK agent loops/UI transports, LangChain bridge work, first-class Tuvren provider packages, ACP, future non-ReAct drivers, future official backends, and future language lines beyond Rust.
