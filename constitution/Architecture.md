...
- **Mitigation status:** The Durable-Read Surface on Framework Shared Services now promotes every durable read that previously justified a private inspector (branch messages, runtime status, turn state, history walks, thread enumeration) onto the host-facing SDK. The Reference Host container explicitly states the SDK-only invariant. The remaining target-state cleanup is Epic AT: delete the playground host package, rename playground-named REPL internals, add headless mode, and add transcript replay. The private-inspector risk is closed for durable reads; the proving-host consolidation risk remains tracked until Epic AT closes.
