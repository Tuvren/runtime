...
- **Total Active Story Points:** 110
- **Critical Path:** `KRT-AD001 -> KRT-AD002 -> KRT-AD003 -> KRT-AD004/KRT-AD005 -> KRT-AD006 -> KRT-AD007 -> KRT-AF001 -> KRT-AF002/KRT-AF003/KRT-AF004/KRT-AF005/KRT-AF006 -> KRT-AF007 -> KRT-AF008`
- **Planning Assumptions:** The checked-in compatibility matrix currently records `typescript-framework` as `full_pass` for `233/233` checks, `typescript-kernel-sqlite` as `full_pass` for `45/45` checks, `typescript-providers` as `full_pass` for `28/28` checks, and `rust-framework` as `unsupported` with `233` non-applicable checks. That proves the promoted subset is green, not that TypeScript is freeze-ready. TechSpec ADR-021 through ADR-029 remain binding, and Rust framework product work stays out of scope until the active freeze-readiness epics close.

### Brownfield Continuity Note

- Epics A-AC are closed in current repo reality through the closure inventories already cited in `constitution/spikes/`.
...
- The active scope is therefore a closure pass over what remains unclassified, implementation-local, structurally risky, or insufficiently conformance-backed before any later framework implementation line is authorized.

### Sequential Scope Rule

- Epics W, X, Y, Z, AA, AB, and AC remain closed and must not be quietly reopened through implementation drift.
- Epics AD, AE, and AF are the only active implementation epics in current repo reality.
...
- **Epic AD — Docs-to-Authority Freeze Gate:** classify every normative framework and kernel docs claim, record whether it is portable, local, deferred, or stale, and clean up constitution/docs drift so Markdown no longer overstates portable semantics.
...
- Rust framework product implementation work is deferred until `KRT-AF008` closes and a later TechSpec/Tasks revision explicitly activates that line.
...
- Future concrete drivers beyond ReAct, official peer backends beyond memory/SQLite, and future product implementation lines beyond the current TypeScript/Rust lanes are deferred until the freeze-readiness gate closes and a later plan explicitly authorizes them.
...
- Authority packets for surfaces beyond the current promoted packet families remain deferred unless Epic AD or AF classifies them as promote-now freeze blockers.
...
- Epics W-AC promoted the first shared semantic authority surfaces, normalized topology, enforced neutral machine authority, closed the TypeScript kernel gap, promoted run-liveness, and promoted framework orchestration.
...
- Epics W-AC closure evidence remains anchored in `constitution/spikes/epic-w-semantic-coverage-matrix.md`, `constitution/spikes/epic-w-semantic-ecosystem-maturity-closure-inventory.md`, `constitution/spikes/epic-x-typescript-topology-normalization-inventory.md`, `constitution/spikes/epic-x-typescript-topology-normalization-closure-inventory.md`, `constitution/spikes/epic-y-authority-leak-inventory.md`, `constitution/spikes/epic-y-machine-enforced-authority-closure-inventory.md`, `constitution/spikes/epic-y-shared-conformance-engine-closure-inventory.md`, `constitution/spikes/epic-z-typescript-kernel-syscall-closure-inventory.md`, `constitution/spikes/epic-aa-kernel-conformance-promotion-closure-inventory.md`, `constitution/spikes/epic-ab-run-liveness-recovery-closure-inventory.md`, `constitution/spikes/epic-ac-framework-orchestration-authority-inventory.md`, and `constitution/spikes/epic-ac-framework-orchestration-authority-closure-inventory.md`.
...
And every implementation-defined or deferred claim is excluded from conformance promotion
