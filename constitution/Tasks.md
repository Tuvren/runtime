...
- **Planning Assumptions:** Epics A-Q are closed in current repo reality. TechSpec v0.5.7 keeps the baseline AI SDK bridge on `LanguageModelV3` / `ProviderV3` from `@ai-sdk/provider@3.0.8`, pins the AG-UI adapter to `@ag-ui/core@0.0.52`, treats tee-based fanout above `ExecutionHandle.events()` as the sanctioned multi-consumer host path when every required tee branch subscribe before the first pull, records SQLite playground validation as a Node-backed path because `@tuvren/backend-sqlite` uses `better-sqlite3`, records the playground-owned aimock/OpenAI E2E lane as local validation rather than a public provider contract, records the manual Gemini lane as an opt-in local proof rather than default automation, and closes the post-ReAct implementation line through `constitution/spikes/epic-q-release-hardening-inventory.md`.
